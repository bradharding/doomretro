...
* The number of sound effects that can play at the same time is now specified using the new `s_channels` CVAR. It is `32` by default.
* The sample rate of sound effects is now specified using the new `s_samplerate` CVAR. It is `44,100` Hz by default.
