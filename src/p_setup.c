...
void P_SetupLevel(int ep, int map)
{
    ...
    compat_corpsegibs = P_GetMapCompatCorpseGibs(ep, map);
    compat_floormove = P_GetMapCompatFloorMove(ep, map);
    compat_light = P_GetMapCompatLight(ep, map);
    compat_limitpain = P_GetMapCompatLimitPain(ep, map);
    compat_nopassover = P_GetMapCompatNoPassOver(ep, map);
    compat_stairs = P_GetMapCompatStairs(ep, map);
    compat_useblocking = P_GetMapCompatUseBlocking(ep, map);
    compat_zombie = P_GetMapCompatZombie(ep, map);
    nograduallighting = P_GetMapNoGradualLighting(ep, map);
