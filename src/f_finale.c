...
static void F_TextWrite(void)
{
    ...
    for (int count = MAX(0, FixedDiv((finalecount - 10) * FRACUNIT, TextSpeed()) >> FRACBITS); count; count--)
    {
        ...
        if (STCFNxxx)
        {
            ...
            V_DrawMenuPatch(cx + 1, cy + 1, hu_font[c], true, false, false);
        }
        else
        {
            if (prev == ' ')
...
static void F_CastPrint(const char *text)
{
    ...
    while (ch)
    {
        ...
        V_DrawMenuPatch(cx + 1, 181, hu_font[c], true, false, false);
...
static void F_BunnyScroll(void)
{
    ...
    patch_t     *p1 = (FREEDOOM || hacx ? W_CacheLastLumpName("PFUB2") : W_CacheWidestLumpName(REKKRSL ? "PFUB2W" : "PFUB2"));
    patch_t     *p2 = (FREEDOOM || hacx ? W_CacheLastLumpName("PFUB1") : W_CacheWidestLumpName(REKKRSL ? "PFUB1W" : "PFUB1"));
    ...
    if (pillarwidth && SCREENWIDTH != NONWIDEWIDTH)
        memset(screens[0], FindDominantEdgeColor(p1, SHORT(p1->height), 16), SCREENAREA);
    ...
    if (finalecount >= 1130)
    {
        ...
        if (finalecount < 1180)
        {
            ...
            V_DrawMenuPatch((VANILLAWIDTH - 104) / 2 + 1, (VANILLAHEIGHT - 64) / 2 + 1,
                (FREEDOOM || hacx ? W_CacheLastLumpName("END0") : W_CacheLumpName("END0")), true, false, false);
            laststage = 0;
        }
        else
        {
            ...
            V_DrawMenuPatch((VANILLAWIDTH - 104) / 2 + 1, (VANILLAHEIGHT - 64) / 2 + 1,
                (FREEDOOM || hacx ? W_CacheLastLumpName(name) : W_CacheLumpName(name)), true, false, false);
        }
    }
}
