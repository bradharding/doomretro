        ...
        if (max)
        {
            ...
            for (int i = 0; i < max; i++)
                P_SpawnBloodSplat(mo->x + (M_BigRandomInt(-radius, radius) << FRACBITS),
                    mo->y + (M_BigRandomInt(-radius, radius) << FRACBITS), color, true, false, floorz, mo);
        }
    }
...
static void P_NightmareRespawn(mobj_t *mobj)
{
    ...
    if (obituaries)
        C_PlayerMessage("%s dead%s%s has respawned.",
            ((mo->flags & MF_FRIEND) && mo->type > MT_NULL && mo->type < NUMMOBJTYPES && monstercount[mo->type] == 1 ? "The" : "A"),
...
//
int P_FindDoomedNum(const int type)
{
    static struct
    {
        int first;
    ...
    if (numfriends >= playerstart - 1)
    {
        ...
        if (gamemap == 1)
            C_Warning(0, "A friendly %s entered this map with %s!",
                mobjinfo[mobj->type].name1, C_GetPlayerName());
        else if (*mobj->name)
            C_Warning(0, "%s followed %s into this map!",
                mobj->name, C_GetPlayerName());
        else
...
void P_SetBloodSplatColor(bloodsplat_t *splat)
{
    ...
    {
        splat->viscolor = colortranslation[GREENBLOOD - 1][REDBLOODSPLATCOLOR];
        splat->viscolfunc = bloodsplatcolfunc;
    }
    else if (r_blood == r_blood_nofuzz)
    {
        splat->viscolor = (splat->color == FUZZYBLOOD
            ? colortranslation[REDBLOOD - 1][REDBLOODSPLATCOLOR] : splat->color);
        splat->viscolfunc = bloodsplatcolfunc;
    }
    ...
    {
        splat->viscolor = colortranslation[REDBLOOD - 1][REDBLOODSPLATCOLOR];
        splat->viscolfunc = bloodsplatcolfunc;
    }
}
