...
static void C_StoreConsoleString(char *dest, const char *src, const size_t dest_size)
{
    ...
    if (GetLocaleInfoA(LOCALE_USER_DEFAULT, LOCALE_IDEFAULTANSICODEPAGE, codepage, sizeof(codepage)) > 1)
    {
        const UINT  consolecodepage = (UINT)atoi(codepage);

        if (consolecodepage && consolecodepage != CP_UTF8)
        {
            WCHAR   wide[CONSOLETEXTMAXLENGTH];
...
void C_ResetWrappedLines(void)
{
    ...
    {
        memset(console[i].wrap, 0, sizeof(console[0].wrap));
...
const kern_t altkern[] =
{
    ...
    { 'f',  'j',  -2 }, { 'F',  'o',  -1 }, { 'f',  't',  -1 }, { 'H',  'j',  -1 },
    { 'h',  '\\', -1 }, { 'h',  'j',  -2 }, { 'I',  'j',  -1 }, { 'i',  'j',  -2 },
    { 'K',  'j',  -1 }, { 'k',  'j',  -2 }, { 'L',  ' ',  -1 }, { 'L',  '"',  -1 },
    { 'L',  '-',  -1 }, { 'L',  '\'', -1 }, { 'L',  '\\', -1 }, { 'L',  'j',  -1 },
    { 'l',  'j',  -2 }, { 'L',  'Y',  -1 }, { 'M',  'j',  -1 }, { 'm',  '"',  -1 },
    { 'm',  '\\', -1 }, { 'm',  'j',  -2 }, { 'N',  'j',  -1 }, { 'n',  '"',  -1 },
    { 'n',  '\'', -1 }, { 'n',  '\\', -1 }, { 'n',  'j',  -2 }, { 'O',  'j',  -1 },
    { 'o',  '"',  -1 }, { 'o',  ',',  -1 }, { 'o',  ';',  -1 }, { 'o',  '\\', -1 },
    { 'o',  'j',  -2 }, { 'p',  '"',  -1 }, { 'P',  ',',  -1 }, { 'p',  ',',  -1 },
    { 'P',  '.',  -1 }, { 'P',  ';',  -1 }, { 'p',  ';',  -1 }, { 'p',  '\\', -1 },
    { 'P',  '_',  -1 }, { 'P',  'J',  -1 }, { 'P',  'j',  -1 }, { 'p',  'j',  -2 },
    { 'R',  'j',  -1 }, { 'r',  ' ',  -1 }, { 'r',  '"',  -1 }, { 'r',  ')',  -1 },
    { 'r',  ',',  -2 }, { 'r',  '.',  -2 }, { 'r',  '/',  -1 }, { 'r',  ';',  -1 },
    { 'r',  '\\', -1 }, { 'r',  '_',  -1 }, { 'r',  'a',  -1 }, { 'r',  'j',  -2 },
    { 'S',  'j',  -1 }, { 's',  ',',  -1 }, { 's',  ';',  -1 }, { 's',  'j',  -2 },
    { 'T',  ' ',  -1 }, { 'T',  ',',  -1 }, { 'T',  '.',  -1 }, { 'T',  ';',  -1 },
    { 'T',  'a',  -1 }, { 'T',  'e',  -1 }, { 'T',  'j',  -1 }, { 't',  'j',  -2 },
    { 'T',  'o',  -1 }, { 't',  't',  -1 }, { 'U',  'j',  -1 }, { 'u',  'j',  -2 },
    { 'V',  ',',  -1 }, { 'v',  ',',  -1 }, { 'V',  '.',  -1 }, { 'v',  '4',  -1 },
    { 'V',  ';',  -1 }, { 'v',  ';',  -1 }, { 'v',  '_',  -1 }, { 'V',  'a',  -1 },
    { 'V',  'j',  -1 }, { 'v',  'a',  -1 }, { 'v',  'j',  -2 }, { 'W',  'j',  -1 },
    { 'w',  'j',  -2 }, { 'X',  'j',  -1 }, { 'x',  '7',  -1 }, { 'x',  'j',  -2 },
    { 'Y',  ',',  -1 }, { 'Y',  '.',  -1 }, { 'Y',  ';',  -1 }, { 'Z',  'j',  -1 },
    { 'z',  'j',  -2 }, { '\0', '\0',  0 }
};
...
static void C_GetWrapPositions(const int index, int wrappositions[CONSOLEWRAPS])
{
    ...
    if (console[index].wrapwidth == CONSOLETEXTPIXELWIDTH)
    {
        memcpy(wrappositions, console[index].wrap, sizeof(console[index].wrap));
        return;
    }

    for (int wrap = 0, start = 0; wrap < arrlen(console[index].wrap) && start < len; wrap++)
    {
    ...
    memcpy(console[index].wrap, wrappositions, sizeof(console[index].wrap));
    console[index].wrapwidth = CONSOLETEXTPIXELWIDTH;
}
...
static int C_GetConsoleDisplayRows(const int index)
{
    ...
    for (int i = 1, prev = 0; i <= arrlen(wraps); i++)
        if (!(wraps[i - 1] > prev && wraps[i - 1] < len))
            return i;
        else
            prev = wraps[i - 1];
    ...
    const patch_t *, const int, const int, const int, const bool, const byte *))
{
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        if (cheatsequence && inputoffset >= 0 && cheatmask[inputoffset + i])
        {
            letter = '\x95';
            nextletter = (cheatmask[inputoffset + i + 1] ? '\x95' : nextletter);
        }
        ...
        else
        {
            ...
            if (letter == '{' && formatting)
            {
                ...
                if (i < len - 5 && !strncasecmp(&text[i + 1], "auto}", 5))
                {
                    autocolor = true;
                ...
                else if (i < len - 5 && !strncasecmp(&text[i + 1], "none}", 5))
                {
                    nocolor = true;
                    bold = true;
                    continue;
        ...
        else
        {
            ...
            if (isdigit(letter) && monospaced)
            {
                V_DrawOverlayTextPatch(screen, screenwidth, x + (letter == '1') - (letter == '4'),
                    y, patch, width - 1, overlaytextcolor, overlaytextshadowcolor, overlaytexttinttab);
                x += zerowidth;
            }
            else
            {
...
static void C_GetOverlayTextColors(void)
{
...
void C_UpdateFPSOverlay(void)
{
        ...
        for (int yy = graphy; yy < graphy + OVERLAYFPSGRAPHHEIGHT + 1; yy++)
            for (int xx = graphx; xx < graphx + OVERLAYFPSGRAPHWIDTH; xx++)
            {
                ...
                else if (dot != PINK1)
                {
                    ...
                    *dest = overlaytexttinttab[(dot << 8) + *dest];
                }
            }
    }
    else
    ...
    V_DrawOverlayTextPatch(screens[0], SCREENWIDTH, x, (graphy += OVERLAYFPSGRAPHHEIGHT + 2) + 3,
...
void C_UpdateTimerOverlay(void)
{
    ...
    C_DrawOverlayText(screens[0], SCREENWIDTH, SCREENWIDTH - timerwidth - OVERLAYTEXTX + 1, y, buffer, true);
}
...
void C_UpdatePlayerPositionOverlay(void)
{
    ...
    C_DrawOverlayText(screens[0], SCREENWIDTH, x - C_OverlayWidth(angle, true), y, angle, true);
    C_DrawOverlayText(screens[0], SCREENWIDTH, x - C_OverlayWidth(coordinates, true),
        y + OVERLAYLINEHEIGHT, coordinates, true);
}
...
void C_UpdatePathOverlay(void)
{
    ...
    if (*distance)
    {
        ...
        C_DrawOverlayText(mapscreen, MAPWIDTH, x - width, y, distance, true);
...
void C_UpdatePlayerStatsOverlay(void)
{
    ...
    C_DrawOverlayText(mapscreen, MAPWIDTH, x - width, y, buffer, true);
    y += OVERLAYLINEHEIGHT + OVERLAYSPACING;
    ...
    if (totalkills)
    {
        ...
        C_DrawOverlayText(mapscreen, MAPWIDTH, x - C_OverlayWidth(kills, false), y, kills, false);
        free(temp1);
        free(temp2);
    ...
    if (totalitems)
    {
        ...
        C_DrawOverlayText(mapscreen, MAPWIDTH, x - C_OverlayWidth(items, false), y, items, false);
        free(temp1);
        free(temp2);
    ...
    if (totalsecrets)
    {
        ...
        C_DrawOverlayText(mapscreen, MAPWIDTH, x - C_OverlayWidth(secrets, false), y, secrets, false);
        free(temp1);
        free(temp2);
    }
}
...
static void UpdateCheatMask(const char *input)
{
    static char command[255];
    int         start = 0;
    int         inputlen = (int)strlen(input);

    memset(cheatmask, false, sizeof(cheatmask));
    ...
    while (start < inputlen)
    {
        ...
        while (end < inputlen && input[end] != ';')
            end++;

        if ((commandlen = end - start) > 0 && commandlen < (int)sizeof(command))
        {
            M_StringCopy(command, input + start, sizeof(command));
            command[commandlen] = '\0';

            if (IsCheatSequence(command))
...
static char *C_GetWrappedTextSegment(const int index, const int start, const int end)
{
    bool    bold = false;
    bool    bolder = false;
    bool    italics = false;
    ...
    char    prefix[5] = "";
    int     prefixlen = 0;
    char    *text = M_SubString(console[index].string, start, (size_t)(end - start));

    for (int i = 0; i < start; i++)
        if (console[index].string[i] == BOLDONCHAR)
        {
            if (bold)
                bolder = true;
            else
        ...
        else if (console[index].string[i] == BOLDOFFCHAR)
        {
            if (bolder)
                bolder = false;
            else
        ...
        else if (console[index].string[i] == ITALICSONCHAR)
            italics = true;
        else if (console[index].string[i] == ITALICSOFFCHAR)
            italics = false;
        else if (console[index].string[i] == MONOSPACEDONCHAR)
            monospaced = true;
        else if (console[index].string[i] == MONOSPACEDOFFCHAR)
            monospaced = false;

    if (bold)
        prefix[prefixlen++] = BOLDONCHAR;

    if (bolder)
        prefix[prefixlen++] = BOLDONCHAR;

    if (italics)
        prefix[prefixlen++] = ITALICSONCHAR;

    if (monospaced)
        prefix[prefixlen++] = MONOSPACEDONCHAR;
    ...
    if (prefixlen)
    {
        ...
        prefix[prefixlen] = '\0';
        temp = text;
        text = M_StringJoin(prefix, text, NULL);
        free(temp);
    }
...
void C_Drawer(void)
{
    ...
    // draw caret
    if (consoleheight && windowfocused && (!messagetoprint || consoleoverlaymenu))
    {
        if (caretwait < tics)
        {
            showcaret = !showcaret;
        ...
        if (showcaret)
        {
            const int   carety = CONSOLEINPUTY - (CONSOLEHEIGHT - consoleheight);
            const int   starty = MAX(carety, 0);
            const int   endy = MIN(carety + CONSOLELINEHEIGHT, SCREENHEIGHT);

            for (int yy = starty; yy < endy; yy++)
            {
                byte    *dest = &screens[0][yy * SCREENWIDTH + x];

                *dest = consolecaretcolor;
                *(dest + 1) = consolecaretcolor;
            }
        }
    }
    else
                        ...
                        || (scrolldirection == 1 && (autocomplete == -1 || *autocompletelist[autocomplete].text)))
                    {
                        ...
                        if (!M_StringCompare(output, input))
                        {
                            ...
                            if ((game == DOOM1AND2
                                ...
                                || (gamemission == pack_masterlevels && game == MASTERLEVELSONLY)
        ...
        if ((ev->data1 & MOUSE_LEFTBUTTON) && usingmouse)
        {
            ...
            if (dragconsolescrollbaractive && scrollbardrawn)
            {
                ...
                if (C_CanScrollOutput())
                {
                    ...
                    if ((position = (int)(dragconsolescrollposition / CONSOLELINEHEIGHT)) >= scrollrange)
                        C_ScrollToBottom();
                    else
                    {
                        C_GetHistoryPositionForVisibleRow(position - 1, &outputhistory, &outputhistoryoffset);
                        scrolloffset = -(int)(dragconsolescrollposition % CONSOLELINEHEIGHT);
                    }
                }
