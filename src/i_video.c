...
void I_SetPalette(byte *playpal)
{
    if (r_color == r_color_max)
    {
        if (r_ditheredlighting)
            for (int i = 0; i < 256; i++)
            {
                colors[i].r = gammatable[gammaindex][*playpal++] & ~3;
                colors[i].g = gammatable[gammaindex][*playpal++] & ~3;
                colors[i].b = gammatable[gammaindex][*playpal++] & ~3;
            }
        else
            for (int i = 0; i < 256; i++)
            {
    ...
    else
    {
        ...
        if (r_ditheredlighting)
            for (int i = 0; i < 256; i++)
            {
                byte    r = gammatable[gammaindex][*playpal++] & ~3;
                byte    g = gammatable[gammaindex][*playpal++] & ~3;
                byte    b = gammatable[gammaindex][*playpal++] & ~3;
