    ...
    if (ev->type == ev_keydown || *consolecheat)
    {
        if ((!menuactive && !paused) || *consolecheat)
        {
            ...
            else if (cht_CheckCheat(&cheat_mus_xy, ev->data2) && !nomusic && musicvolume)
            {
                ...
                {
                    const int   ep = (gamemode == commercial ? gameepisode : buffer[0] - '0');
                    const int   map = (gamemode == commercial ? (buffer[0] - '0') * 10 + buffer[1] - '0' : buffer[1] - '0');
...
void ST_Ticker(void)
{
    ...
    else if (r_hud && r_hud_style != r_hud_style_alternate)
    {
        ST_UpdateFaceWidget();
        st_oldhealth = viewplayer->health;
    }
