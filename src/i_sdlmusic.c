...
{
    if (timidity_cfg_path[0])
        if (M_FileExists(timidity_cfg_path))
            C_Output("Using TiMidity configuration file %s.", uppercase(timidity_cfg_path));
        else
            C_Warning("Can't find TiMidity configuration file %s.", uppercase(timidity_cfg_path));
}
