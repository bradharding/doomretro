...
#define LIQUIDROCKFACTOR(diff)  ((diff) * 3 / 2)
                    ...
                    switch (sprtemp[frame].rotate)
                    {
                        ...
                        case 1:
                            ...
                            for (int rot = 0; rot < 16; rot++)
                                if (sprtemp[frame].lump[rot] == -1)
                                    I_Error("R_InitSpriteDefs: Frame %c of sprite %.8s is missing rotations",
                                        frame + 'A', sprnames[i]);
...
static inline void GetSpriteTiltScale(const fixed_t basescale, const fixed_t baseiscale,
    const fixed_t tz, const fixed_t tiltfactor, const fixed_t w, fixed_t *scale, fixed_t *iscale)
{
    const fixed_t   tzcol = tz + FixedMul(w, tiltfactor);

    if (tzcol > (FRACUNIT >> 4))
    {
        *scale = FixedDiv(projection, tzcol);
        *iscale = FixedDiv(tzcol, projection);
    }
    else
    {
        *scale = basescale;
...
static void R_DrawVisSprite(const vissprite_t *vis)
{
    ...
    if (r_sprites_tilt || vis->liquidrockfactor)
    {
        const fixed_t   tr_x = vis->gx - viewx;
        const fixed_t   tr_y = vis->gy - viewy;
...
//
// R_DrawVisSpriteClipped
//
static void R_DrawVisSpriteClipped(const vissprite_t *vis)
{
    fixed_t         frac = vis->startfrac;
    const fixed_t   xiscale = vis->xiscale;
    const fixed_t   x2 = vis->x2;
    ...
    if (r_percolumnlighting && !vis->fullbright && !fixedcolormap && (flags & (MF_SHOOTABLE | MF_CORPSE | MF_SPECIAL)))
    {
        ...
        for (dc_x = vis->x1; dc_x <= x2; dc_x++, frac += xiscale, pcl_gx += pcl_dx, pcl_gy += pcl_dy)
        {
            ...
            if ((dc_numposts = column->numposts))
            {
                ...
                if (subsector != lastsubsector)
                {
                    R_UpdatePerColumnLighting(subsector, pcl_lightindex, true, flags, vis, mobj, &black);
                    lastsubsector = subsector;
                }

                if (flooratcolumn >= gz - 8 * FRACUNIT)
                {
                    const int64_t   shadowtopscreen = (int64_t)centeryfrac
                                        - FixedMul(flooratcolumn + mobj->shadowoffset - viewz, spryscale);
    ...
    else
    {
        ...
        for (dc_x = vis->x1; dc_x <= x2; dc_x++, frac += xiscale)
        {
            ...
            if ((dc_numposts = column->numposts))
            {
                ...
                if ((flooratcolumn = R_PointInSubsector(colgx, colgy)->sector->interpfloorheight) >= gz - 8 * FRACUNIT)
                {
                    const int64_t   shadowtopscreen = (int64_t)centeryfrac
                                        - FixedMul(flooratcolumn + mobj->shadowoffset - viewz, spryscale);
                    int             numposts = dc_numposts;
...
//
// R_DrawVisSpriteClippedWithShadow
//
    ...
    else
    {
        ...
        if (fixedcolormap)
        {
            ...
            vis->sectorcolormap = (!ISINVULNERABILITYCOLORMAP(viewplayer->fixedcolormap) ?
                R_GetSectorColormap(sec) : nocolormap);
        }
        else
        {
