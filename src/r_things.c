    ...
    for (i = 0; i < numsprites; i++)
    {
        ...
        if (j >= 0)
        {
            ...
            do
            {
                ...
                {
                    R_InstallSpriteLump(j + firstspritelump, lump->name[4] - 'A', 
                        lump->name[5] - '0', false);
                    if (lump->name[6])
                        R_InstallSpriteLump(j + firstspritelump, lump->name[6] - 'A',
                           lump->name[7] - '0', true);
                }
...
void R_ProjectSprite(mobj_t *thing)
{
    ...
    if (thing->flags2 & MF2_FEETARECLIPPED && thing->z <= thing->subsector->sector->floorheight)
        vis->footclip = MIN((spriteheight[lump] >> FRACBITS) / 5, 10);
    else
        vis->footclip = 0;
