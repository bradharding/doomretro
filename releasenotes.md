...
* Full support has been added for John Romero’s newly-released megawad [*SIGIL*](https://www.romerogames.ie/sigil/). When loading the *The Ultimate DOOM’s* IWAD from the WAD launcher, if `sigil.wad` is in the same folder, it will automatically be loaded and a fifth entry, “Sigil of Baphomet”, will appear in the episode menu.
...
* The graphic detail will no longer be affected by the value of the `r_screensize` CVAR when changed in the options menu and not playing a game.
