...
* The additional motion blur effect applied when the player is injured and the `r_shakescreen` CVAR is `on` has been improved further, and will only be applied when the `vid_motionblur` CVAR is also `on`.
