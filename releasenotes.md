...
* The corpses of some of the smaller monsters can now be gibbed in reaction to nearby barrel and rocket explosions. This feature can be toggled on or off using the new `r_corpses_gib` CVAR, which is `on` by default and `off` when vanilla mode is on.
