...
* These changes have been made to [*ID24*](https://doomwiki.org/wiki/ID24) compatibility in `DEHACKED` lumps:
  * `ID24 bits`, `Min respawn tics`, `Respawn dice`, `Pickup ammo type`, `Pickup ammo category`, `Pickup weapon type`, `Pickup item type`, `Pickup bonus count`, `Pickup sound`, `Pickup message` and `Translation` are now supported for things.
  * `Tranmap` is now supported for states.
