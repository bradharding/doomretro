...
* The infighting among monsters after the player dies can now be toggled using the new `infighting` CVAR. This CVAR is `on` by default and `off` when vanilla mode is enabled.
