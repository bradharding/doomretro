...
* When displaying a lump from a PWAD that is only 320 pixels wide, and the `vid_widescreen` CVAR is `on`, the color of the pillarboxes will now be based on the most used color along the left and right edges of that lump, rather than always be black.
