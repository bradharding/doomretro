...
* The file `midiproc.exe` that allows the music volume to be adjusted independently of the sound effects volume is no longer necessary and therefore no longer included with *DOOM Retro*.
...
* The player no longer makes a sound when trying to open a door that is currently open.
...
* The behavior of friendly monsters has been improved. They no longer always crowd the player when there are no other monsters around.
