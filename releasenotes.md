...
* These changes have been made to the automap:
  * Sectors can now be filled with floor textures or colors using the new `am_sectors` CVAR, which can be set to `textures`, `colors` or `off` (the default).
  ...
  * The player’s path when the `am_path` CVAR is `on` is now anchored to the back of the player’s arrow rather than the center.
