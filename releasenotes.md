...
* A crosshair can now be displayed by enabling the new `crosshair` CVAR. It is `off` both by default and when vanilla mode is on, and requires the `mouselook` CVAR to be `on` and the `autoaim` CVAR to be `off`. The crosshair may be customized by including a `DRXHAIR` lump in a PWAD.
