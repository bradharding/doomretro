...
* Motion blur is now properly applied to the player’s view when they look up and down and the `mouselook` CVAR is `on` and the `vid_motionblur` CVAR is greater than `0%`.
