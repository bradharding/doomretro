...
* When displaying a lump on the title screen from a PWAD that is only 320×200 in size but the `vid_widescreen` CVAR is `on`, the color of the pillarboxes on either side will be based on the colors on the left and right edges of that lump.
