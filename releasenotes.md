...
* When the `vid_widescreen` CVAR is `on`, if the screen is wider than the status bar, the `GRNROCK` or `FLOOR7_2` textures no longer fill the sides. A color based on the colors in the status bar is used instead.
