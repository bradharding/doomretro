...
* The default of the `am_allmapwallcolor` CVAR is now `106` and the default of the `am_allmapcdwallcolor` CVAR is now `108`.
* Lines colored with the `am_tswallcolor` CVAR are no longer displayed in the automap when the player has a computer area map power-up.
