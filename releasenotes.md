...
* If it doesn’t already exist, *DOOM Retro* now creates an `autoload` folder at startup. Any `.wad`, `.deh` or `.bex` file that is placed in this folder, (or in a subfolder based on the name of the currently loaded WAD), will then always be automatically loaded at startup.
...
* The following changes have been made to the support of `DEHACKED` lumps:
  ...
  * `Blood color` may now be used to change the color of blood spilled by monsters. This is set to a value between `0` to `8` (representing the colors red, gray, green, blue, yellow, black, purple, white and orange). For a monster to spill fuzzy blood, the `SHADOW` flag must be used in `Bits` and the `r_blood` CVAR must be `all`.
